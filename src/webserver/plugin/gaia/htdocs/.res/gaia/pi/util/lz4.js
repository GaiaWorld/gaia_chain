...
var lz4_asm_1 = require("./lz4_asm");
/**
 * 压缩
 * @return Uint8Array，压缩后的数据，用完后需要调用free释放内存
 */
exports.compress = function (src) {
    var compressBound = LZ4_compressBound(src.length);
    var compressHeap = newU8Heap(compressBound);
    var compressSize = LZ4_compress_default(src, compressHeap.byteOffset, src.length, compressBound);
    if (compressSize <= 0) {
        // 压缩出错
 ...
 * @return Uint8Array，压缩后的数据，用完后需要调用free释放内存
 */
exports.decompress = function (src, decompressSize) {
    var decompressHeap = newU8Heap(decompressSize);
    var realSize = LZ4_decompress_safe(src, decompressHeap.byteOffset, src.length, decompressSize);
    if (realSize <= 0) {
...
var LZ4_compress_default = lz4_asm_1.LZ4ASM.cwrap('LZ4_compress_default', 'number', ['array', 'number', 'number', 'number']);
/*! LZ4_decompress_safe() :
...
// tslint:disable-next-line:variable-name
var LZ4_decompress_safe = lz4_asm_1.LZ4ASM.cwrap('LZ4_decompress_safe', 'number', ['array', 'number', 'number', 'number']);
/*!
LZ4_compressBound() :
...
// tslint:disable-next-line:variable-name
var LZ4_compressBound = lz4_asm_1.LZ4ASM.cwrap('LZ4_compressBound', 'number', ['number']);
/**
 * 创建属于asmjs的堆对应的U8Array，用完后需要显示调用free释放内存
 * @param size 分配大小
 */
var newU8Heap = function newU8Heap(size) {
    var ptr = lz4_asm_1.LZ4ASM._malloc(size);
    var r = new Uint8Array(lz4_asm_1.LZ4ASM.HEAPU8.buffer, ptr, size);
    return r;
};
})